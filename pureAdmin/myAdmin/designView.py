...
def dataList(info):
    ...
    try:
        ...
        if wjId:
            ...
            temp_detail.append("提交时间")
            temp_question_id_list = list()
            for question in questions:
                temp_question_id_list.append(question.id)
                temp_detail.append(question.title)
            detail.append(temp_detail)
            for submit in submit_data_list:
                temp_detail_submit = list()
                submit_id = submit[0]
                submit_time = submit[1].strftime('%Y/%m/%d %H:%M:%S')
                temp_detail_submit.append(submit_id)
...
# 根据submit_id获取
def getOptBySubmitId(id):
    cursor = connection.cursor()
    cursor.execute(
        'SELECT ma.questionId, COALESCE(mo.title, ma.answerText) as opt '
        'FROM myAdmin_answer ma LEFT JOIN myAdmin_options mo ON ma.answer = mo.id WHERE ma.submitId = %s' % id)
